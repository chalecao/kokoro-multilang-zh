...
/**
 * Runs ESpeakNg with the given arguments and returns the phonemized text.
 *
...
async function espeakRun(text, lang) {
  const espeakResult = await ESpeakNg({
    arguments: [
      "-v", lang,
      "--ipa=3",
      "--phonout", "generated",
  ...
  const phenoms = espeakResult.FS.readFile("generated", { encoding: "utf8" });
  return phenoms.replace(/(\r\n|\n|\r)/g, ' ').trim();
}
